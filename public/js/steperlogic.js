$(document).ready(function() {
  ...
  $("#submit_everything").click(function(){
   var type = $('[name="type_delivery"]').val(),
   weight = $('#shipment_weight').val(),
   material_type= $('#material_type').val();
   
   var first_name_detail = $('#first_name').val(),
      last_name_detail= $('#last_name').val(),
      city_detail= $('#city').val(),
      state_detail= $('#state').val(),
      source=$('#source_value').val(),
      country_detail= $('#country').val();
   var first_name_detail_dest = $('#first_name_dest').val(),
   last_name_detail_dest= $('#last_name_dest').val(),
   destination_value_detail_dest= $('#destination_value').val(),

   cont_info_detail_dest= $('#cont_info').val();
   $.ajax({
      ...
      data: JSON.stringify({
         ...
         first_name_detail: first_name_detail,
         last_name_detail: last_name_detail,
         city_detail: city_detail,
         state_detail: state_detail,
         country_detail: country_detail,
         first_name_detail_dest: first_name_detail_dest,
         last_name_detail_dest: last_name_detail_dest,
         destination_value_detail_dest: destination_value_detail_dest,
         cont_info_detail_dest: cont_info_detail_dest,
         source:source,
         created_id: $("#user_id").val()
      }),
      statusCode: {
         200: function() {
            alert("Shipment Successfully Registered");
  ...
  $('#submit1').click(function(e){
   ...
   source=$('#source_value').val(),
   country= $('#country').val();
   if(first_name && last_name && city && state && country && source){
      return true;
   ...
   $('#submit2').click(function(e){
      // Extract the checked checkboxes from the first step
      var first_name = $('#first_name_dest').val(),
      last_name= $('#last_name_dest').val(),
      destination_value= $('#destination_value').val(),

      cont_info= $('#cont_info').val();
      if(first_name && last_name && cont_info && destination_value){
         return true;
      }   alert("All fields are manditory")
         return false
      });
      $('#submit3').click(function(e){
         ...
         if(type && weight && material_type){
            $("#recipient_details_section").html(`
               <p>First name: ${first_name_detail}</p>
               <p>Last name: ${last_name_detail}</p>
               <p>City: ${city_detail}</p>
               <p>State: ${state_detail}</p>
               <p>Country: ${country_detail}</p>
               <p>Source: ${source}</p>
            `);
            $("#destination_details_section").html(`
               <p>First name: ${first_name_detail_dest}</p>
               <p>Last name: ${last_name_detail_dest}</p>
               <p>Destination: ${destination_value_detail_dest}</p>
               <p>Contact Info: ${cont_info_detail_dest}</p>
                
            `);
            $("#shipment_details_section").html(`
               <p>Type: ${type}</p>
               <p>Weight: ${weight}</p>
               <p>Material Type: ${material_type && material_type.toUpperCase()}</p>
            `);
            var material_cost= $('#material_type option:selected').attr('cost');
            var destination_cost = $('#destination_value option:selected').attr('cost');
            $("#cost_summary").text(material_cost);
            $("#delivery_cost_summary").text(destination_cost);
            $("#total").text(parseInt(material_cost) + parseInt(destination_cost));
            return true;
         }   
         alert("All fields are manditory")
            return false
...
function validateStepOne() {
  ...
  state= $('#state').val(),
  country= $('#country').val();
  if(first_name && last_name && city && state && country && source_value){
     return true;
  }
